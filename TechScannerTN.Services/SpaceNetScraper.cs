...
public class SpaceNetScraper : IWebScraper
{
    private readonly ILogger<SpaceNetScraper> _logger;
    private readonly HttpClient _httpClient;

    public string ProviderName => "SpaceNet";

    public SpaceNetScraper(ILogger<SpaceNetScraper> logger)
    {
        _logger = logger;
    ...
    public async Task<List<Plan>> ScrapeAsync()
    {
        ...
        try
        {
            _logger.LogInformation("Starting SpaceNet scraping");
            
            var response = await _httpClient.GetAsync("https://www.spacenet.tn");
            if (!response.IsSuccessStatusCode)
            {
                _logger.LogWarning("Failed to fetch SpaceNet page. Status code: {StatusCode}", response.StatusCode);
                return plans;
            }
            ...
            var planNodes = doc.DocumentNode.SelectNodes("//div[@class='plan'] | //div[contains(@class, 'offer')] | //div[contains(@class, 'package')]");

            if (planNodes == null || planNodes.Count == 0)
            {
                _logger.LogWarning("No plan nodes found on SpaceNet page");
                return plans;
            }
            ...
            foreach (var node in planNodes)
            {
                ...
                {
                    _logger.LogWarning(ex, "Error extracting plan from SpaceNet node");
                }
            }

            _logger.LogInformation("SpaceNet scraping completed. Found {PlanCount} plans", plans.Count);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error scraping SpaceNet");
        }
